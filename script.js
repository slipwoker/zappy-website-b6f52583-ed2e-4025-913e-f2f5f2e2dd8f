  ...
  if (contactForm) {
    ...
    function getErrorContainer(field) {
      ...
      if (!errorEl) {
        ...
        if (id) errorEl.setAttribute('id', id + '-error');
    ...
    function getErrorMessage(field, rule) {
      ...
      return field.dataset['error' + rule.charAt(0).toUpperCase() + rule.slice(1)] || messages[rule] || 'Invalid value.';
    }
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('required') || field.dataset.required === 'true') {
        if (!VALIDATORS.required(value)) {
          valid = false;
          message = getErrorMessage(field, 'required');
        }
      }

      if (valid && field.type === 'email' && value.trim() !== '') {
        if (!VALIDATORS.email(value)) {
          valid = false;
          message = getErrorMessage(field, 'email');
        }
      }

      if (valid && field.type === 'tel' && value.trim() !== '') {
        if (!VALIDATORS.tel(value)) {
          valid = false;
          message = getErrorMessage(field, 'tel');
        }
      }

      if (valid && field.dataset.minlength && value.trim() !== '') {
        if (!VALIDATORS.minlength(value, field.dataset.minlength)) {
          valid = false;
          message = getErrorMessage(field, 'minlength');
        }
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "בלונוש משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'b6f52583-ed2e-4025-913e-f2f5f2e2dd8f',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'b6f52583-ed2e-4025-913e-f2f5f2e2dd8f';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'b6f52583-ed2e-4025-913e-f2f5f2e2dd8f';
